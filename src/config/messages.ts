...
export const MAP_TEXT = {
  ...
  picker: {
    ...
    affiliationOff: '非表示',
    /**
     * 人を掴んで動かせるかどうかの切り替え。既定は「表示のみ」。
     *
     * スマホでは掴んだ瞬間に領域・関係線を引き直す処理が重く、一瞬表示が崩れて
     * 見えることがある。ほとんどの人は配置を変えずに眺めるだけなので、
     * 動かせる状態を選んだときだけそのコストを払う形にする。
     */
    editMode: '編集',
    editModeOn: '編集できる',
    editModeOff: '表示のみ',
  ...
  card: {
    map: {
      ...
      note: [
        ...
        '複数の所属は領域の重なりで表れます。',
        '人を押すと紹介へのリンクを出します。',
        '配置を変えたいときは「表示設定」の編集を「編集できる」にすると、人を掴んで動かせます（領域と関係線も追従します）。',
      ].join(''),
