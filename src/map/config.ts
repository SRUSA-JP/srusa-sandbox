...
export const NODE = {
   ...
   * ノード同士の間隔。
   *
   * 名前がアイコンより横に長いので、アイコンではなく名前が重ならない幅で決める。
   * どれだけ詰められるかは check:map の「名前の重なり」が出す空きを見ること。
   * いまは同じ行でいちばん近い名前どうしに 22px の空きが残る。
   */
  gapX: 92,
  gapY: 56,
...
export const FLOORPLAN = {
   ...
   * いちばん長い関係線をどれだけ嫌うか。
   *
   * 総距離だけを縮めると、多くの線を少し縮める代わりに 1 本だけ図を
   * 突っ切る形が選ばれる。図を横切る線は 1 本でも目立つので、
   * いちばん長い 1 本も点数に入れる。
   */
  longestWeight: 4,
   ...
   * 落ち着き、線の長さが大きく振れる。中身の広さから出した幅のまわりを
   * いくつか試して、いちばん良かったものを採る。
   */
  minColumns: 6,
  /** 試す幅の刻み（升目の数）。 */
  widthStep: 3,
   ...
   * 図が細長いことをどれだけ嫌うか。
   *
   * 0 だと縦一列に積んだ形が選ばれてしまう（配線の総距離を図の大きさで
   * 割っているので、縦に伸ばすほど点数が良くなるため）。画面に収まるのは
   * 正方形に近い形なので、縦横の比が 1 から離れるほど点数を悪くする。
   */
  aspectWeight: 1.5,
} as const;
