...
- 原文: `オフラインカスタムとか面白クリップの追加で`
  - オフラインカスタム（対面で集まって行うカスタムゲーム・イベント）の記録機能を追加する
  ...
  - 原文: `APEXオフラインカスタムこれ https://youtube.com/live/CQt4xY8o_0A?feature=share 20230305が日付`
    - 記録対象の第2例（APEX）。日付は 2023-03-05
    - 面白クリップのギャラリー（`config/clips.ts`）とは別物として扱う。ゲームの名シーンではなく
      対面カスタムイベントの記録なので、実装時はオフラインカスタムの記録機能側に取り込む
    - 参加者・場所などの詳細は本文からは分からないため、着手時に確認する
  - 原文: `年表も適宜更新で`
    - 上の 2 例の日付は、[data/srusa-history-v0.1.json](data/srusa-history-v0.1.json) の `オフラインカスタム`（2026-03-27）
      / `APEX カスタム`（2023-03-05）の該当ぶんに反映済み（それぞれもう 1 回ぶんは未記入のまま残す）。
      オフラインカスタムの記録機能そのものを作るときも、年表に反映すべき事実が増えたら同様に追記する
  - 原文: `適宜オフラインの写真はマスクかマイクラキャラとかで置換して貼る予定`
    - オフラインカスタムの記録機能に写真を持たせるときは、原本の顔写真をそのまま貼らない前提で設計する
      （利用者側でマスク処理・Minecraft キャラクター等への置換をしてから渡す運用）
    - CLAUDE.md「個人情報の扱いに注意する」の匿名化方針に合わせ、置換前の生データを持ち込まない
  - 原文: `これAPEXカスタム https://youtube.com/live/CQt4xY8o_0A?feature=share
    https://www.youtube.com/live/zpi7LkDHyKU これもオフラインカスタムの別配信`
    - 1本目は第2例（APEX、2023-03-05）と同じ URL。重複のため新規データなし
    - 2本目は新しい記録対象（第3例）。「オフラインカスタムの別配信」とのことなので、
      `オフラインカスタム`（一般枠、2 回開催のうちもう 1 回ぶん）の配信アーカイブとして扱う。
      年月は本文からは分からないため未記入のまま、アーカイブの存在だけ年表に反映した
    - 参加者・場所・具体的な年月はまだ分からないため、着手時に確認する
  - 原文: `これ2025830のオフラインカスタム https://www.youtube.com/live/zpi7LkDHyKU
    https://www.youtube.com/live/HxJ8X9AKKlc?si=grR-6mQfp3Aucjj4`
    - 第3例の日付が判明。2025-08-30。年表の `オフラインカスタム`（もう 1 回ぶん）に反映済み
    - 2本目の URL も同日ぶんのアーカイブとして扱う（配信アーカイブ 2 本、年表の detail に反映）
    - 参加者・場所はまだ分からないため、着手時に確認する
...
- 原文: `スマホでノードを移動すると一瞬表示が崩れる、編集モードを追加してデフォルトオフで，すなわちデフォルトはビューモードで`
  - 相関図に「編集」の切り替え（表示設定）を追加した。既定は「表示のみ」
  - `RelationshipMap` に `editable`（既定 false）を追加。false のときは人を押して
    紹介を見ることはできるが、掴んで動かす操作（座標の書き換え）はしない
  - スマホで表示が崩れて見えていたのは、掴んだ瞬間に領域・関係線を毎回引き直す
    処理が重いため。既定を表示のみにしたことで、ほとんどの人はこのコストを
    払わずに済むようになった（動かしたい人は「編集できる」を選ぶ）
  - 確認: `npm run build` が通ること。Playwrightで表示のみ（既定）のときは
    ドラッグしても座標が変わらないこと・押すと紹介が出ることを確認し、
    「編集できる」を選ぶと今までどおり掴んで動かせることも確認した

- 原文: `相関図にこのアルゴリズムで作るってのを追加して` / `試しに作ってみて`（仕様は前回のTODOにそのまま残していたものを実装）
  - 「関連度重力」という配置・ドラッグ追従モードを追加した（`src/map/config.ts` の `LAYOUT_MODES`）
  - エッジの重み w は `src/map/layout.ts` の `buildGravityEdges()` で作る。手動関係は 1 + 共有タグ希少度、
    共有タグのみのペアは希少度そのもの。希少度は「その所属を共有する人が少ないほど大きい」（`1 / (人数-1)`）
  - 静止レイアウト（`placeGravity()`）は理想エッジ長・ハブ判定の反発力・中心への求心力の 3 つの力を
    積み上げ、ドラッグ追従と同じ時間刻み・減衰・速度上限（`GRAVITY_PHYSICS`）で釣り合うまで動かす
  - ドラッグ追従は `src/map/gravityDrag.ts`（引力の伝搬・追従相手の決定、純関数）と
    `src/hooks/useGravityFollow.ts`（requestAnimationFrame での速度積分）に分けた。
    引力の伝搬は「掛けて減っていく重みの中で最大値を採る」問題なので、ダイクストラ法の
    足す/小さいを掛ける/大きいに置き換えた版として実装した
  - `requestAnimationFrame` は必ず 60fps とは限らない（ヘッドレス環境ではもっと速く呼ばれることがある）ため、
    実際に経過した時間を 60fps の何フレームぶんかに換算してから力・減衰・上限を掛けるようにした
  - 発散するバグを見つけて直した: `withPositions()` が呼ぶ `enforceStrictRegions()`（領域からはみ出した
    非所属者を 1 回あたり最大数千 px 押し出す処理）が、追従で動かす複数人を毎フレーム相手にすると
    追従のバネ力と押し合って発散し、無関係な人まで巻き込んで座標が数万 px 単位で吹き飛んでいた。
    `withPositions()` に `enforceRegions` オプションを足し、gravity モードのときだけ押し出しをやめて
    現在位置から囲いを引き直すだけにした（他の配置は今までどおり）
  - 確認: `npm run build` が通ること。Playwright で実際にドラッグし、掴んだ人・引力の強い追従相手・
    無関係な人（それぞれ 79 人ぶんの座標を比較）の移動量を数値で確認した
    （掴んだ人 747.6px、追従相手 678.0px、無関係な人 0.0px）
  - **未確認: ブラウザでの見え方の作り込み。** 発散は直したが「試しに作ってみて」の依頼どおりの
    トライアル実装で、静止レイアウトの見た目（ばね定数・繰り返し回数）や追従の感触は詰めていない
...
- 原文: `それ目次更新も折りたたむか要約して`
  ...
  - [DESIGN.md](DESIGN.md) に、ホームの更新ログとページ目次は要約優先・詳細折りたたみで扱う規格を追記した

- 原文: `ヘッダーは折りたたみも出来るように，マップとかゲームタイトルとか`
  - 固定ヘッダー下段のコンテキスト切替に開閉ボタンを追加し、ゲーム名やマップなどの切替タブを必要なときだけ表示できるようにした
  - 閉じた状態でも `ゲーム / Minecraft / ワールドマップ` のような現在地の要約を残し、親ジャンルから子画面への包含関係が分かるようにした
  - [DESIGN.md](DESIGN.md) に、ヘッダー下段の子切替は要約と開閉ボタンを持つ規格を追記した
...
- 原文: `バグってる まだ表示の縦枠小さい，`
  ...
  - UI 初期レンダリング検査と [DESIGN.md](DESIGN.md) に、3D ビューアの枠潰れ防止方針を追加した

- 原文: `維持費で書くプレイ屋のプレイ時間も`
  - 維持費パネルの本文側に `プレイヤー別プレイ時間` を追加し、請求額の計算に使っている各プレイヤーの時間を見えるようにした
  - カスタム対象を選んだときにも、そのプレイヤーのプレイ時間を入力欄の近くに表示するようにした
  - UI 初期レンダリング検査と [DESIGN.md](DESIGN.md) の維持費規格に、プレイ時間を画面上にも残す方針を追加した

- 原文: `ホームniに各ページの目次．リンクで`
  - ホームのジャンプ欄を `ページ目次` に改め、各ページへ直接移動できるリンク一覧として見えるようにした
  - 各リンクに説明文とハッシュ URL の補助ラベルを添え、Minecraft / ボドゲ / 人 / 記録 / ギャラリーの包含関係を維持した
  - [DESIGN.md](DESIGN.md) に、ホームのページ目次は [src/content/home.ts](src/content/home.ts) で管理する規格を追記した

- 原文: `フッターのアイコン流石に分かりにくいな．前のロゴのアイコンをフィルタ欠けるとかそういう感じで作ってみて．` / `領域の背景の色は重なりがすごいからデフォルトオフで．カテゴリを橋梁表示するときにその領域の背景に色を付ける感じで．` / `ツールチップの表示はデフォルトオフで` / `ツールチップはやっぱ今はクリックしたら出る情報がツールチップで出るように` / `凡例を折りたためるように` / `関係木でカテゴリごとにある程度距離を取るように．今は全部が近いから` / `ツールチップ？クリックで表示されるカテゴリの文字の色をカテゴリと統一して` / `この情報の表示オフにしていらない` / `スマホでフッターにまとめすぎてる，一列だけで，マップとかゲームの切替はヘッダーで`
  - スマートフォン下部ナビのアイコンを、元の線アイコンを角張らせた `PixelNavIcon` に変更し、親ジャンルの意味を読み取りやすくした
  - スマートフォン下部ナビは親ジャンル 1 列だけに戻し、ゲーム内・マップ内などの子切替は固定ヘッダー側に出すようにした
  - 相関図は領域背景と SVG 標準 tooltip を初期オフにし、人物クリック時の吹き出しでカテゴリと関係者を確認する形にした
  - 領域背景がオフでも、凡例や下部凡例でカテゴリを強調したときはそのカテゴリの領域だけ薄く表示するようにした
  - 相関図内の凡例を折りたたみ式にし、開いたときだけカテゴリ一覧を表示するようにした
  - 関係樹では同じ深さの人物をカテゴリ単位で少し離し、nodoame 中心の枝分かれが詰まりにくいようにした
  - 人物クリックの吹き出しでは、カテゴリ名の文字色をカテゴリ色に揃えた
  - 年表ページの「いま分かっていること」統計カードを表示しないようにした

- 原文: `後は相関図の表示を軽量化して動作として．`
  - 相関図の関係者名と共有カテゴリの計算を、各ノード描画時の都度走査から `RelationshipMap` 内のメモ化マップへ移した
  - ピクセル顔の SVG を 64 個の `rect` ではなく、同色セルをまとめた少数の `path` で描くようにして DOM ノード数を減らした
  - 画像アバター以外では人物ノードごとの `defs` / `clipPath` を出さないようにした
  - UI 初期レンダリング検査に相関図 HTML サイズの上限を追加し、肥大化の回帰を検出できるようにした

- 原文: `アイコン最初のに戻して`
  - スマートフォン下部ナビの `PixelNavIcon` を、元の線アイコン加工版から最初の 5×5 ドット絵パターンへ戻した
  - UI 初期レンダリング検査と [DESIGN.md](DESIGN.md) の下部ナビ規格も 5×5 ドット絵前提に戻した

- 原文: `アイコン戻ってない．`
  - 「最初のアイコン」はドット絵導入前の通常線アイコンと解釈し、スマートフォン下部ナビも `Icon` を使う形へ戻した
  - UI 初期レンダリング検査と [DESIGN.md](DESIGN.md) / [CLAUDE.md](CLAUDE.md) の下部ナビ規格も通常線アイコン前提へ更新した

- 原文: `相関図とかのデータを整理したい，関係が多い人を順に中心においていってそこから枝分かれ，して樹木状？に広がっていく感じで表示するモードを追加してデフォルトにしてみて`
  / `nodoame nodoamen? を必ず中心に`
  / `でいったんnodoame の繋がっているカテゴリを整理して．`
  / `nodoamenn = nodoameね`
  / `OctbeeとかNとかNatchとかmitigliaとかは複数nodoameとれんけいしてるからそういうのも必ずあった設計になるように`
  - 相関図の配置モードに `関係樹` を追加し、初期表示をこのモードにした
  - `関係樹` は `nodoame` を必ず中心根にし、明示関係と `groupConnects()` が true の所属から導いた繋がりで外へ枝分かれさせる
  - nodoame と繋がるカテゴリを共有する Octbee / N / natch / mitiglia が、明示関係が少なくても中心根の枝として拾われる設計にした
  - `nodoamen` / `nodoamenn` は Minecraft 側や表記揺れの名前として、相関図データでは `nodoame` の `aliases` に追加した
  - `Mitigli4` / `Mitfli4` は Minecraft 側や表記揺れの名前として、相関図データでは `mitiglia` の `aliases` に追加した
  - メンバー一覧カードに、カテゴリだけでなく相関図上でつながりのある人の名前を最大 4 人まで表示するようにした
  - 相関図の人物 tooltip に、関係している人の名前を最大 6 人まで表示し、残りは件数で省略するようにした
  - 明示関係線は、共有しているグループがある場合にグループ色の並行線として表示するようにした
  - 相関図の中にもコンパクト凡例を重ね、クリックでグループ強調を切り替えられるようにした
  - 似ている名前候補を機械的に確認し、未統合候補として `arata/creata`、`hika/rik`、`nomu/noto`、`nomu/tomy` を確認対象にした
  - `小学校` のような学校段階や `アクティブメンバー` のような状態札だけでは繋がりを作らない方針を [DESIGN.md](DESIGN.md) と [CLAUDE.md](CLAUDE.md) に追記した
  - UI 初期レンダリング検査と相関図検査に、`関係樹`、`nodoame`、`nodoamenn` alias、代表者の共有カテゴリ確認を追加した
...
- 原文: `マイクラはマイクラでコンテンツまとめてね` / `統計と、マップとか、カレンダー APEXとかとは粒度違うから`
  ...
  - （後日 `gameId` / `gameLabel` によるゲームナビの作り直しが入り、`group` はそのフォールバックとして残っている）
...
- 原文: `プレビューデプロイできていなう`
  ...
  - （後日 Netlify の GitHub App 連携が別途有効化されており、PR ごとのデプロイプレビューは `deploy-preview` ジョブと関係なく動いている）
