...
- 相関図に「関連度ベースの重力アルゴリズム」という配置・ドラッグ追従モードを追加する。原文（仕様）をそのまま残す:

  > 関連度ベースの重力アルゴリズム
  >
  > エッジの重み w は関連度(手動リンクは 1+共有タグ希少度、共有タグのみから
  > 推測されたリンクは希少度そのもの)。
  >
  > 【静止レイアウト】
  > ・理想エッジ長 = 60 / √max(w, 1) … 重いエッジほど短くまとまる
  > ・ノード反発力 = 次数がハブ閾値(5)以上なら90000、未満なら6000(距離の2乗に反比例)
  > ・中心への求心力 = 0.7
  >
  > 【ドラッグ追従(掴んだノードにつられて動く)】
  > 1. 1本のエッジの引力 e(w) = w / (w + 4) … 0〜1未満に飽和する曲線
  > 2. 掴んだノードを起点に幅優先探索(最大3ホップ)し、経路上の e(w) を掛け合わせて
  >    伝搬。複数経路がある場合は最大値を採用し、0.02未満は打ち切り
  > 3. 直接つながる隣接ノード(0ホップ目)は引力を×1.5(上限0.99)にブースト
  > 4. 引力が強い上位16ノードを追従対象とし、掴んだ瞬間の相対オフセットを記録
  > 5. 毎フレーム、各追従ノードに次の力を加算して速度積分で位置を更新:
  >    ・バネ力 = (オフセット込みの目標位置 − 現在位置) × (引力 × 3)
  >    ・反発力 = 0.08 × ノード反発力 / 距離²(全ノードとの重なり回避)
  >    ・速度 = (旧速度 + 合力 × 0.02) × 0.78(減衰)、最大40px/frameでクランプ
  >
  > 以上の数式と定数だけで、「関連が強いほど強く・近いほど速く引き合う」重力的な
  > 挙動を再現できる。

  - 実装するときは `src/map/config.ts` の `LAYOUT_MODES` に追加し、静止配置は `src/map/layout.ts`、
    ドラッグ追従は `RelationshipMap` 側の掴んで動かす処理に組み込む想定
  - 重み w（関連度）の定義は既存の共有タグ希少度の算出箇所を再利用できるか確認すること
