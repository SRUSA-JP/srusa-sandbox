...
ボドゲのスコア記録画面は、紙の得点表に近い「行＝回・カテゴリ」「列＝参加者」を主役にする。
入力表を順位表より先に置き、表の中に列見出し、行見出し、合計行、行合計を入れる。
広い表ではヘッダーと左端の項目列を固定し、横スクロールの枠を表だけに閉じる。
空欄は `0` ではなく未入力として保存し、入力状況を表の直上に出す。
HTML の表では `caption` と `scope` を付け、読み上げでもどの人・どの回の得点か分かるようにする。

### ナビゲーションの包含関係

画面の親ジャンルは `routes.ts` の `navGroup` を正にする。PC ヘッダー、スマートフォン下部ナビ、
ホームのジャンプ一覧は同じ親ジャンルを使う。`section` はページ内の古いまとまりや互換用の情報で、
新しく導線を作るときは `navGroup` を見る。

スマートフォンでは下部ナビを親ジャンルだけにし、現在の親ジャンル内の切替は固定ヘッダー側に置く。
ゲームは `ゲーム名 → そのゲーム内の画面`、人は `メンバー / 相関図`、記録は `年表 / イベント` の順にする。
下部ナビのアイコンは通常の `Icon` を使い、ヘッダーや操作ボタンと意味・形を揃える。

### プレイ時間の扱い

公開用の日別プレイ時間は `scripts/build-player-daily-summary.mjs` でログの `joined the game` と
`left the game` をペアにして再計算する。`first_seen_jst` から `last_seen_jst` までの差は途中の離席を
含むため、プレイ時間として使わない。退出が無いセッションは過大計上を避けて閉じず、サーバー停止ログが
あればその時刻で閉じる。複数日にまたがる集計区間では、セッションと区間の交差時間だけを足す。
...
領域背景と SVG 標準 tooltip は初期オフにし、人物をクリックした吹き出しだけで詳細を出す。
領域背景オフのときも、凡例や下部凡例からカテゴリを強調した場合はそのカテゴリの領域だけ薄く出す。
人物のピクセル顔は 64 個の `rect` ではなく、同色セルをまとめた `path` で描く。相関図の初期 HTML は
`scripts/check-ui-render.tsx` の上限を超えないようにし、関係者名など全体走査が必要な表示情報は
`RelationshipMap` 側で一度だけ作ってから各ノードへ渡す。
